...
def _reward(entities, jet):
    ...
    # Exponential penalty for altitude < 1000m
    if alt < 1000.0:
        penalty_factor = (1000.0 - alt) / 1000.0  # 0.0 at 1000m, 1.0 at 0m
        r -= 5.0 * (np.exp(penalty_factor * 3.0) - 1.0) * DT
    
    # Exponential penalty for X/Z boundary violations > 18,000m (maxing at 20k)
    box_size = 20000.0  # Half-size of simulation box
    x_violation = max(0.0, abs(p[0]) - box_size)
    z_violation = max(0.0, abs(p[2]) - box_size)
    max_violation = 2000.0  # Max violation at 20k
    if x_violation > 0.0:
        violation_factor = min(x_violation / max_violation, 1.0)
        r -= 10.0 * (np.exp(violation_factor * 2.0) - 1.0) * DT
    if z_violation > 0.0:
        violation_factor = min(z_violation / max_violation, 1.0)
        r -= 10.0 * (np.exp(violation_factor * 2.0) - 1.0) * DT
    
    # 3-phase reward structure: Phase 1 (no missiles), Phase 2 (1 missile), Phase 3 (3 missiles)
    phase = max(1, min(3, int(REWARD_PHASE)))  # Clamp to 1-3
    ms = _missiles(entities, jet)
    
    # Apply missile-specific rewards based on phase
    if phase == 2:  # Phase 2: Process 1 missile
        ms_filtered = ms[:1]  # Only consider closest missile
        jp = p
    ...
    elif phase >= 3:  # Phase 3: Process up to 3 missiles
        ms_filtered = ms[:3]  # Consider up to 3 closest missiles
        jp = p
        for d, _, _, mv, mp in ms_filtered:
            rel = jp - mp
            dn = float(np.linalg.norm(rel))
...
def _save():
    try:
        ...
        with open(_MODEL_ACTOR,'wb') as f:
            pickle.dump(_actor.get_weights(),f,protocol=4)
        with open(_MODEL_Q1,'wb') as f:
            pickle.dump(_q1.get_weights(),f,protocol=4)
        with open(_MODEL_Q2,'wb') as f:
            pickle.dump(_q2.get_weights(),f,protocol=4)
        with open(_MODEL_Q1T,'wb') as f:
            pickle.dump(_q1t.get_weights(),f,protocol=4)
        with open(_MODEL_Q2T,'wb') as f:
            pickle.dump(_q2t.get_weights(),f,protocol=4)
        with open(_MODEL_LOG_ALPHA,'wb') as f:
            pickle.dump(float(_log_alpha.numpy()),f,protocol=4)
        with open(_MODEL_BUF,'wb') as f:
            pickle.dump({'s':_s,'a':_a,'r':_r,'s2':_s2,'d':_d,'ptr':_ptr,'sz':_sz},f,protocol=4)
        with open(_MODEL_STEPS,'wb') as f:
            pickle.dump(_steps,f,protocol=4)
        with open(_MODEL_UPD,'wb') as f:
            pickle.dump(_upd,f,protocol=4)
    except Exception:
        pass
...
def _load():
    ...
    try:
        # Try loading from new separate files first
        if _MODEL_ACTOR.exists():
            with open(_MODEL_ACTOR,'rb') as f:
                _actor.set_weights(pickle.load(f))
        if _MODEL_Q1.exists():
            with open(_MODEL_Q1,'rb') as f:
                _q1.set_weights(pickle.load(f))
        if _MODEL_Q2.exists():
            with open(_MODEL_Q2,'rb') as f:
                _q2.set_weights(pickle.load(f))
        if _MODEL_Q1T.exists():
            with open(_MODEL_Q1T,'rb') as f:
                _q1t.set_weights(pickle.load(f))
        if _MODEL_Q2T.exists():
            with open(_MODEL_Q2T,'rb') as f:
                _q2t.set_weights(pickle.load(f))
        if _MODEL_LOG_ALPHA.exists():
            with open(_MODEL_LOG_ALPHA,'rb') as f:
                _log_alpha.assign(float(pickle.load(f)))
        if _MODEL_BUF.exists():
            with open(_MODEL_BUF,'rb') as f:
                b=pickle.load(f)
            if isinstance(b,dict) and all(k in b for k in ('s','a','r','s2','d','ptr','sz')):
                ...
                _sz=int(b.get('sz',0))
        if _MODEL_STEPS.exists():
            with open(_MODEL_STEPS,'rb') as f:
                _steps=int(pickle.load(f))
        if _MODEL_UPD.exists():
            with open(_MODEL_UPD,'rb') as f:
                _upd=int(pickle.load(f))
        return
    except Exception:
        pass
